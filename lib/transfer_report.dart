...
import 'package:alquds_sarraf/transfer_card.dart';
...
class TransferReport extends StatefulWidget {
    ...
    static const String id = 'transfer_report';
  TransferReport({Key? key}) : super(key: key);
...
for(var item in _loadedPhotos){
 ...
 DateTime itemDate2 =DateTime.parse("${itemDate.year.toString()}-${itemDate.month.toString().padLeft(2,'0')}-${itemDate.day.toString().padLeft(2,'0')}");
   ...
   if(itemDate2.compareTo(parsedDate) >= 0 && itemDate2.compareTo(parsedDate2) <= 0) {
      if( item['currency'].toString() == "شيكل")
       blnc += double.parse( item['cost'].toString());
       else if( item['currency'].toString() == "دينار")
       blnc_dn += double.parse( item['cost'].toString());
         else if( item['currency'].toString() == "دولار")
       blnc_dl += double.parse( item['cost'].toString());
         else if(item['currency'].toString() == "يورو")
       blnc_eu += double.parse( item['cost'].toString());
    results.add(item);
    print( "3");
   }
 }
          ...
          physics: const ClampingScrollPhysics(), 
            ...
            itemBuilder: (BuildContext context, int index) { 
                  return TransferCard(
                                        ...
                                        f_id: _loadedPhotos2[index]['f_id'] ?? "",
                                        f_name: _loadedPhotos2[index]['f_name'] ?? "",
                                         t_id: _loadedPhotos2[index]['t_id'] ?? "",
                                        t_name: _loadedPhotos2[index]['t_name'] ?? "",
